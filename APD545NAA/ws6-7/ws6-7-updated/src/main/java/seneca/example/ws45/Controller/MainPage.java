...
package seneca.example.ws45.Controller;
import seneca.example.ws45.Model.*;
import seneca.example.ws45.Util.FileManager;
import seneca.example.ws45.Util.DatabaseManager;
...
public class MainPage {
    ...
    void onAddPart(ActionEvent event) throws IOException {
        openWindow("/seneca/example/ws45/add-part.fxml", "Add Part");
    }
    ...
    void onModifyPart(ActionEvent event) throws IOException {
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/seneca/example/ws45/modify-part.fxml"));
    ...
    void onAddProduct(ActionEvent event) throws IOException {
        openWindow("/seneca/example/ws45/add-product.fxml", "Add Product");
    }
    ...
    void onModifyProduct(ActionEvent event) throws IOException {
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/seneca/example/ws45/modify-product.fxml"));
        Parent root = loader.load();
    ...
    private void handleSaveToFile(ActionEvent event) {
        try {
            FileManager.saveDataToFile("inventory.dat");
    ...
    private void handleLoadFromFile(ActionEvent event) {
        try {
            FileManager.loadDataFromFile("inventory.dat");
    ...
    private void handleSaveToDB(ActionEvent event) {
        try {
            ...
            showAlert("Success", "Data saved to database.");
        } catch (Exception e) {
            showAlert("Error", "Failed to save to database: " + e.getMessage());
        }
    }

    @FXML
    private void handleLoadFromDB(ActionEvent event) {
        try {
            DatabaseManager.loadFromDatabase();
            showAlert("Success", "Data loaded from database.");
        } catch (Exception e) {
            showAlert("Error", "Failed to load from database: " + e.getMessage());
        }
    }
